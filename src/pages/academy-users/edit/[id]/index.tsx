...
import { getAcademyUserById, updateAcademyUserById } from 'apiSdk/academy-users';
...
function AcademyUserEditPage() {
  const router = useRouter();
  const id = router.query.id as string;
  const { data, error, isLoading, mutate } = useSWR<AcademyUserInterface>(
    () => (id ? `/academy-users/${id}` : null),
    () => getAcademyUserById(id),
  );
  ...
  const handleSubmit = async (values: AcademyUserInterface, { resetForm }: FormikHelpers<any>) => {
    setFormError(null);
    try {
      const updated = await updateAcademyUserById(id, values);
      mutate(updated);
      resetForm();
    } catch (error) {
      setFormError(error);
